...
def search(request):
	print "in search"
	if 'q' in request.GET:
		result = query(q=request.GET['q'],fl="fsmTitle,fsmRelatedTitle,fsmDateCreated,fsmImageUrl,fsmCreator,fsmTypeOfResource,fsmNote,fsmIdentifier,fsmPhysicalLocation,fsmTeiUrl")['response']
		return HttpResponse(json.dumps(result), content_type="application/json")
	else:
